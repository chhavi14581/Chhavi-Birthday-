document.addEventListener("DOMContentLoaded", async () => {
  ...
  const scaleContent = () => {
    const baseDimensions = { width: 1100, height: 745 };
  ...
  const timeElapse = (date) => {
    ...
    const result = `PRO. DAY : <span class="digit">${days}</span> days <span class="digit">${hours}</span> hours <span class="digit">${minutes}</span> minutes <span class="digit">${seconds}</span> seconds`;
